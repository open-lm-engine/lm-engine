...
# Supported model sizes; each may have a logs-{size}-long directory
MODEL_SIZES = ("7b", "400m")

# Base path for log directories (logs-7b-long, logs-400m-long, ...)
...
# All use "score,none" which is the unified metric
BENCHMARKS = [
    # Summarization
    ("longbench_gov_report", "score,none", "GovReport"),
    ("longbench_qmsum", "score,none", "QMSum"),
    ("longbench_multi_news", "score,none", "MultiNews"),
    # Few-shot
    ("longbench_trec", "score,none", "TREC"),
    ("longbench_triviaqa", "score,none", "TriviaQA"),
    ("longbench_samsum", "score,none", "SAMSum"),
    # Code
    ("longbench_lcc", "score,none", "LCC"),
    ("longbench_repobench-p", "score,none", "RepoBench-P"),
]
...
AVERAGE_COLUMNS = [
    (
        ...
        True,
    ),  # Average of selected LongBench benchmarks (higher is better)
]
...
def extract_model_name(dir_name: str, size: str) -> str:
    """Extract model name from directory name like 'results-mamba2-400m-cosine'."""
...
def format_value(value: float, metric_key: str) -> str:
    ...
    # All LongBench scores are 0-1, show as percentage with 1 decimal place
...
def generate_latex_table(model_results: dict[str, dict], size: str = "400m") -> str:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        f"\\caption{{{size_display} LongBench Evaluation Results}}",
        f"\\label{{tab:{size}-long-results}}",
    ...
    # Find best average values per group (for bolding within each midrule section)
    ...
    for i, model in enumerate(models):
        ...
        # Add average columns (only bold these)
...
def main():
    for size in MODEL_SIZES:
        logs_dir = LOGS_BASE / f"logs-{size}-long"
