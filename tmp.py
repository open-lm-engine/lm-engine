...
class MSU(nn.Module):
    def __init__(
        ...
        if kernel_size is None:
            ...
            self.activation_string is not None and is_glu(self.activation_string)
        ...
        self.log_forget_inv_temp = nn.Parameter(
            torch.zeros((self.num_k_heads, self.k_state_head_dim)) + temp_logistic_p[:, None]
        )
        ...
        self.output_projection = ParameterizedLinear(
            self.num_q_heads * self.v_state_head_dim, self.output_size, bias=False, std=std
        )
    ...
    def forward(
        ...
        if self.low_rank is None:
            ...
            if self.is_gated_normalization:
                input, gate = input.split(
                    (self.total_intermediate_size, self.num_q_heads * self.v_state_head_dim), dim=-1
                )
        else:
            if self.is_gated_normalization:
        ...
        reset_input = torch.full_like(expanded_input[..., :, 0], fill_value=20.0)
        ...
        expanded_output = expanded_output.view(
            B, self.k_state_head_dim, S, self.num_v_heads, self.v_state_head_dim
        ).permute(0, 2, 3, 1, 4)
