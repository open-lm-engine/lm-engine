...
# Supported model sizes; each may have a logs-{size}-retrieval directory
MODEL_SIZES = ("7b", "400m")

# Base path for log directories (logs-7b-retrieval, logs-400m-retrieval, ...)
...
BENCHMARKS = [
    ("squad_completion", "contains,none", "SQuAD"),
    ("based_nq_2048", "contains,none", "NQ"),
    ("based_drop", "contains,none", "DROP"),
    ("based_triviaqa", "contains,none", "TriviaQA"),
    ("fda", "contains,none", "FDA"),
    ("swde", "contains,none", "SWDE"),
]
...
AVERAGE_COLUMNS = [
    (
        ...
        True,
    ),  # Average of all retrieval benchmarks (higher is better)
]
...
def format_value(value: float, metric_key: str) -> str:
    ...
    # All retrieval metrics are accuracy-like, show as percentage with 1 decimal place
...
def generate_latex_table(model_results: dict[str, dict], size: str = "400m") -> str:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        f"\\caption{{{size_display} Retrieval Evaluation Results}}",
        f"\\label{{tab:{size}-retrieval-results}}",
...
def main():
    for size in MODEL_SIZES:
        logs_dir = LOGS_BASE / f"logs-{size}-retrieval"
