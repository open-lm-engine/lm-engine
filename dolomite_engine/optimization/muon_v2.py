...
class Muon(torch.optim.Optimizer):
            ...
            for p in params:
                ...
                if getattr(p, "_is_expert_weight", False):
                    ...
                    if isinstance(g, DTensor):
                        ...
                        # g = g.full_tensor()
                        g = g.to_local() # TODO: this step doesn't involve reducing gradients across DDP ranks 
    ...
    #                 met_data_dtensor_ew= None
    #                 # met_data_dtensor_eg = None

    #                 if isinstance(expert_weights_, DTensor):
    #                     met_data_dtensor_ew = dict(
    #                         placements=expert_weights_.placements,
    #                         device_mesh=expert_weights_.device_mesh,
    #                     )
    #                     expert_weights_ = expert_weights_.to_local()
                    
    #                 if isinstance(expert_grads_, DTensor):
    #                     # met_data_dtensor_eg = dict(
    #                     #     placements=expert_grads_.placements,
    #                     #     device_mesh=expert_grads_.device_mesh,
    #                     # )
    #                     expert_grads_ = expert_grads_.to_local()
    ...
    #                 if "momentum_buffer" not in state:
    #                     # state["momentum_buffer"] = [torch.zeros_like(expert_grads_[i]) for i in range(num_experts)]                    
    #                     state["momentum_buffer"] = torch.zeros_like(expert_grads_)        
                    
    #                 buf = state["momentum_buffer"]
    #                 buf.mul_(momentum).add_(expert_grads_)
                    
    #                 if group["nesterov"]:
    #                     expert_grads_ = expert_grads_.add(buf, alpha=momentum) 
    #                 else:
    #                     expert_grads_ = buf

    #                 u = zeropower_via_newtonschulz5_batched(expert_grads_, steps=group["ns_steps"])
    #                 # scale update
    #                 adjusted_lr = adjust_lr_wd_for_muon(lr, matched_adamw_rms,expert_weights_.shape,is_expert=True)
    #                 # Apply weight decay
    #                 expert_weights_.data.mul_(1 - lr * wd)

    #                 # Apply update for the expert weight
    #                 expert_weights_.data.add_(u, alpha=-adjusted_lr)
                    
    #                 if met_data_dtensor_ew is not None:
    #                     # update underlyind data in-place so p.data instead of just p 
    #                      p.data = DTensor.from_local(
    #                             expert_weights_,
    #                             device_mesh=met_data_dtensor_ew["device_mesh"],
    #                             placements=met_data_dtensor_ew["placements"]
    #                         ) # TODO: check if we want to Dtensor the gradients also 
    #                     # p.data = distribute_tensor(expert_weights_, device_mesh=met_data_dtensor_ew["device_mesh"], placements=met_data_dtensor_ew["placements"])
    ...
    #                 num_experts = expert_weights_.shape[0]
    ...
    #                     # scale update
    #                     adjusted_lr = adjust_lr_wd_for_muon(lr, matched_adamw_rms,expert_weight.shape)
    #                     # Apply weight decay
