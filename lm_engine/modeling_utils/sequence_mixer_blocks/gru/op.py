...
def gru(
    ...
    *,
    ...
    """
    computes multihead GRU recurrent update over the sequence length
    ...
    :param forget_input: forget input tensor of shape (B, S, Nxf, H) where Nxf is the number of forget input
        heads and H is the head dimension. Should have shape (T, Nxf, H) and `cu_seqlens` should be passed.
    :type forget_input: torch.Tensor
    :param forget_weight: forget weight tensor of shape (Nwf, H, H)
    :type forget_weight: torch.Tensor
    :param reset_input: reset input tensor of shape (B, S, Nxr, H) where Nxr is the number of reset input heads
        and H is the head dimension. Should have shape (T, Nxr, H) and `cu_seqlens` should be passed.
    :type reset_input: torch.Tensor
    :param reset_weight: reset weight tensor of shape (Nwr, H, H)
    :type reset_weight: torch.Tensor
    :param input_state: starting state of shape (B, N, H), where N = max{Nx, Nxf, Nxr, Nw, Nwf, Nwr}. None means
        starting state is 0 tensor. Defaults to None.
