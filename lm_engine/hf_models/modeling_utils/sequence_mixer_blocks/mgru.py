...
if is_fma_available():
    ...
    from fma.modules.mgru import mgru
...
class MGRU(nn.Module):
    def __init__(
        ...
        if kernel_size is None:
            ...
            self.conv1d = ParameterizedConv1d(
                in_channels=3 * self.intermediate_size,
                out_channels=(6 if is_glu_activation else 3) * self.intermediate_size,
    ...
    def forward(
        ...
        input, forget_input, reset_input = [
            i.view(*input.size()[:-1], self.num_heads, self.intermediate_head_dim)
            for i in (input, forget_input, reset_input)
        ]
