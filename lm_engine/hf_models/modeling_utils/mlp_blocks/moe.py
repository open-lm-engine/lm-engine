...
class MoE(DTensorModule):
    def __init__(
        ...
        use_interleaved_weights_for_mlp: bool,
        use_interleaved_weights_for_moe: bool,
        ...
        self.use_interleaved_weights_for_mlp = use_interleaved_weights_for_mlp
        self.use_interleaved_weights_for_moe = use_interleaved_weights_for_moe
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if is_kernel_allowed(Kernel.sonicmoe):
            assert self.use_interleaved_weights_for_moe
    ...
    def _compute_experts(
        ...
        if is_kernel_allowed(Kernel.scattermoe):
            ...
            x = self.act(x, is_interleaved=self.use_interleaved_weights_for_moe) if self.is_glu else self.act(x)
    ...
    def _compute_shared_experts(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = self.act(x, is_interleaved=self.use_interleaved_weights_for_mlp) if self.is_glu else self.act(x)
