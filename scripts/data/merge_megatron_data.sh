...
for ITEM in "${INPUT_PATHS[@]}"; do
    INPUT_PATH="/data/tmp3/dolma3-pool-tokenized/data/$ITEM"
    OUTPUT_PATH="/data/tmp4/dolma3-pool/$ITEM"
    
    ray job submit --no-wait --address http://localhost:8267 -- bash -c "cd lm-engine && uv pip install -e . && MSC_CONFIG=/app/lm-engine/configs/msc/gcs.yml python tools/data/merge_data.py --input-directory $INPUT_PATH --output-prefix $OUTPUT_PATH --max-size 250 --msc-base-path mayank-data --tmpdir /local-ssd --download-locally"
