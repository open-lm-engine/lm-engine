...
def _attn_fwd_inner(
    ...
    for start_n in tl.range(lo, hi, BLOCK_N):
        ...
        k = tl.load(
            K_b_ptr
            + (k_stride[1] * off_kvh_idxs + k_stride[2] * off_n_idxs)[:, None]
            + k_stride[3] * head_dim_idxs[None, :]
        ).T
        v = tl.load(
            V_b_ptr
            + (v_stride[1] * off_kvh_idxs + v_stride[2] * off_n_idxs)[:, None]
            + v_stride[3] * head_dim_idxs[None, :]
        )
        qk = tl.dot(q, k)
        if STAGE == 2:
...
def _attn_fwd(
    ...
    Q_ptr = _maybe_make_tensor_desc(
        Q_ptr, shape=[y_dim, HEAD_DIM], strides=[HEAD_DIM, 1], block_shape=[BLOCK_M, HEAD_DIM]
    )
    K_b_ptr = K_ptr + k_stride[0] * off_b
    V_b_ptr = V_ptr + v_stride[0] * off_b
    O_ptr = _maybe_make_tensor_desc(
        O_ptr, shape=[y_dim, HEAD_DIM], strides=[HEAD_DIM, 1], block_shape=[BLOCK_M, HEAD_DIM]
    )
