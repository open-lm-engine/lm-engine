# **************************************************
# Copyright (c) 2026, Mayank Mishra
# **************************************************

import pytest
import torch

from lm_engine.kernels import KernelBackend
from lm_engine.training.enums import Kernel
from lm_engine.training.generation_cache import GenerationCache
from lm_engine.training.kernels import enable_kernels
from lm_engine.training.modeling_utils.sequence_mixer_blocks import GRU, GRUArgs, gru
from tests.layers.utils import assert_equal_tensors, get_duplicated_tensors, skip_if_incompatible_kernel_backend


_HIDDEN_SIZE = 32
_BATCH = 2
_PREFILL_LEN = 16
_SEED = 42


def _get_problem_shapes() -> list[tuple[int, int, int, int, int, int, int]]:
    # (state_head_dim, num_input_heads, num_forget_input_heads, num_reset_input_heads,
    #  num_weight_heads, num_forget_weight_heads, num_reset_weight_heads); each shape perturbs a
    # single head-count away from a common base, one at a time, so every dimension gets exercised
    # through the grouped-head repeat_interleave broadcasting at least once
    base = [8, 4, 4, 4, 4, 4, 4]

    result = [(9, 7, 7, 7, 7, 7, 7)]
    for i in range(1, len(base)):
        t = base.copy()
        t[i] = 8
        result.append(tuple(t))

    return result


_PROBLEM_SHAPES = _get_problem_shapes()
_DTYPES = [torch.float32, torch.float16]


def _make_gru(
    device: torch.device, dtype: torch.dtype, problem_shape: tuple[int, int, int, int, int, int, int]
) -> GRU:
    (
        state_head_dim,
        num_input_heads,
        num_forget_input_heads,
        num_reset_input_heads,
        num_weight_heads,
        num_forget_weight_heads,
        num_reset_weight_heads,
    ) = problem_shape

    config = GRUArgs(
        state_head_dim=state_head_dim,
        num_input_heads=num_input_heads,
        num_forget_input_heads=num_forget_input_heads,
        num_reset_input_heads=num_reset_input_heads,
        num_weight_heads=num_weight_heads,
        num_forget_weight_heads=num_forget_weight_heads,
        num_reset_weight_heads=num_reset_weight_heads,
        add_bias=False,
        normalization_function="rmsnorm",
        gradient_clipping=None,
        kernel_size=4,
        activation_function="silu",
    )

    torch.manual_seed(42)
    gru_module = GRU(
        input_size=_HIDDEN_SIZE,
        output_size=_HIDDEN_SIZE,
        config=config,
        initializer_range=0.02,
        m_width=1.0,
        init_method="normal",
        num_layers=1,
        layer_idx=0,
        use_depth_scaled_init=False,
        use_padding_free_transformer=False,
    ).to(device=device, dtype=dtype)
    gru_module.eval()

    return gru_module


@pytest.mark.parametrize("problem_shape", _PROBLEM_SHAPES)
@pytest.mark.parametrize("dtype", _DTYPES)
def test_triton_prefill_vs_torch_prefill(
    dtype: torch.dtype, problem_shape: tuple[int, int, int, int, int, int, int]
) -> None:
    device = skip_if_incompatible_kernel_backend(KernelBackend.triton)
    gru_module = _make_gru(device, dtype, problem_shape)

    torch.manual_seed(0)
    x = torch.randn(_BATCH, _PREFILL_LEN, _HIDDEN_SIZE, device=device, dtype=dtype)

    with enable_kernels([Kernel.gru]):
        out_k = gru_module(x)

    out_f = gru_module(x)

    assert_equal_tensors(out_k, out_f, False)


@pytest.mark.parametrize("problem_shape", _PROBLEM_SHAPES)
@pytest.mark.parametrize("dtype", _DTYPES)
def test_triton_decode_vs_torch_decode(
    dtype: torch.dtype, problem_shape: tuple[int, int, int, int, int, int, int]
) -> None:
    device = skip_if_incompatible_kernel_backend(KernelBackend.triton)
    gru_module = _make_gru(device, dtype, problem_shape)

    torch.manual_seed(0)
    x = torch.randn(_BATCH, _PREFILL_LEN, _HIDDEN_SIZE, device=device, dtype=dtype)
    x_gen = torch.randn(_BATCH, 1, _HIDDEN_SIZE, device=device, dtype=dtype)

    with enable_kernels([Kernel.gru]):
        cache_k = GenerationCache()
        gru_module(x, cache_params=cache_k)
        out_gen_k = gru_module(x_gen, cache_params=cache_k)

    cache_f = GenerationCache()
    gru_module(x, cache_params=cache_f)
    out_gen_f = gru_module(x_gen, cache_params=cache_f)

    assert_equal_tensors(out_gen_k, out_gen_f, False)


@pytest.mark.parametrize("problem_shape", _PROBLEM_SHAPES)
@pytest.mark.parametrize("dtype", _DTYPES)
@pytest.mark.parametrize("has_input_state", [False, True])
def test_torch_prefill_continuation(
    dtype: torch.dtype, problem_shape: tuple[int, int, int, int, int, int, int], has_input_state: bool
) -> None:
    torch.manual_seed(0)

    (
        state_head_dim,
        num_input_heads,
        num_forget_input_heads,
        num_reset_input_heads,
        num_weight_heads,
        num_forget_weight_heads,
        num_reset_weight_heads,
    ) = problem_shape
    num_heads = max(
        num_input_heads,
        num_forget_input_heads,
        num_reset_input_heads,
        num_weight_heads,
        num_forget_weight_heads,
        num_reset_weight_heads,
    )

    batch_size, sequence_length = 2, 10
    split = 4

    x = torch.randn(batch_size, sequence_length, num_input_heads, state_head_dim, dtype=dtype)
    xf = torch.randn(batch_size, sequence_length, num_forget_input_heads, state_head_dim, dtype=dtype)
    xr = torch.randn(batch_size, sequence_length, num_reset_input_heads, state_head_dim, dtype=dtype)
    W = torch.randn(num_weight_heads, state_head_dim, state_head_dim, dtype=dtype)
    Wf = torch.randn(num_forget_weight_heads, state_head_dim, state_head_dim, dtype=dtype)
    Wr = torch.randn(num_reset_weight_heads, state_head_dim, state_head_dim, dtype=dtype)
    h0 = torch.randn(batch_size, num_heads, state_head_dim, dtype=dtype) if has_input_state else None

    def _run(x, xf, xr, h0):
        return gru(
            input=x,
            weight=W,
            forget_input=xf,
            forget_weight=Wf,
            reset_input=xr,
            reset_weight=Wr,
            input_state=h0,
            kernel_backend=KernelBackend.torch,
        )

    full_output, full_state = _run(x, xf, xr, h0)

    first_output, first_state = _run(x[:, :split], xf[:, :split], xr[:, :split], h0)
    second_output, second_state = _run(x[:, split:], xf[:, split:], xr[:, split:], first_state)

    chunked_output = torch.cat([first_output, second_output], dim=1)

    assert_equal_tensors(chunked_output, full_output, False)
    assert_equal_tensors(second_state, full_state, False)


def _get_op_level_tensors(
    device: torch.device,
    dtype: torch.dtype,
    problem_shape: tuple[int, int, int, int, int, int, int],
    batch_size: int,
    sequence_length: int,
) -> tuple[dict, dict]:
    (
        state_head_dim,
        num_input_heads,
        num_forget_input_heads,
        num_reset_input_heads,
        num_weight_heads,
        num_forget_weight_heads,
        num_reset_weight_heads,
    ) = problem_shape

    x_kernel, x_torch = get_duplicated_tensors(
        (batch_size, sequence_length, num_input_heads, state_head_dim), device=device, dtype=dtype
    )
    xf_kernel, xf_torch = get_duplicated_tensors(
        (batch_size, sequence_length, num_forget_input_heads, state_head_dim), device=device, dtype=dtype
    )
    xr_kernel, xr_torch = get_duplicated_tensors(
        (batch_size, sequence_length, num_reset_input_heads, state_head_dim), device=device, dtype=dtype
    )
    W_kernel, W_torch = get_duplicated_tensors(
        (num_weight_heads, state_head_dim, state_head_dim), device=device, dtype=dtype
    )
    Wf_kernel, Wf_torch = get_duplicated_tensors(
        (num_forget_weight_heads, state_head_dim, state_head_dim), device=device, dtype=dtype
    )
    Wr_kernel, Wr_torch = get_duplicated_tensors(
        (num_reset_weight_heads, state_head_dim, state_head_dim), device=device, dtype=dtype
    )

    kernel = dict(
        input=x_kernel,
        weight=W_kernel,
        forget_input=xf_kernel,
        forget_weight=Wf_kernel,
        reset_input=xr_kernel,
        reset_weight=Wr_kernel,
    )

    torch_ref = dict(
        input=x_torch,
        weight=W_torch,
        forget_input=xf_torch,
        forget_weight=Wf_torch,
        reset_input=xr_torch,
        reset_weight=Wr_torch,
    )

    return kernel, torch_ref


@pytest.mark.parametrize("is_compiling", [False, True])
@pytest.mark.parametrize("has_input_state", [False, True])
@pytest.mark.parametrize("problem_shape", _PROBLEM_SHAPES)
@pytest.mark.parametrize("dtype", _DTYPES)
@torch._dynamo.config.patch(recompile_limit=1024)
def test_op_forward_kernel_vs_torch(
    dtype: torch.dtype,
    problem_shape: tuple[int, int, int, int, int, int, int],
    has_input_state: bool,
    is_compiling: bool,
) -> None:
    device = skip_if_incompatible_kernel_backend(KernelBackend.triton)
    torch.manual_seed(_SEED)

    state_head_dim = problem_shape[0]
    num_heads = max(problem_shape[1:])

    kernel, torch_ref = _get_op_level_tensors(device, dtype, problem_shape, batch_size=4, sequence_length=32)

    h0_kernel = h0_torch = None
    if has_input_state:
        h0_kernel, h0_torch = get_duplicated_tensors((4, num_heads, state_head_dim), device=device, dtype=dtype)

    gru_fn = torch.compile(gru, fullgraph=True) if is_compiling else gru

    y_kernel, h_kernel = gru_fn(**kernel, input_state=h0_kernel, kernel_backend=KernelBackend.triton)
    y_torch, h_torch = gru(**torch_ref, input_state=h0_torch, kernel_backend=KernelBackend.torch)

    assert_equal_tensors(y_kernel, y_torch, False)
    assert_equal_tensors(h_kernel, h_torch, False)


@pytest.mark.parametrize("is_compiling", [False, True])
@pytest.mark.parametrize("has_input_state", [False, True])
@pytest.mark.parametrize("problem_shape", _PROBLEM_SHAPES)
@pytest.mark.parametrize("dtype", _DTYPES)
@torch._dynamo.config.patch(recompile_limit=1024)
def test_op_backward_kernel_vs_torch(
    dtype: torch.dtype,
    problem_shape: tuple[int, int, int, int, int, int, int],
    has_input_state: bool,
    is_compiling: bool,
) -> None:
    device = skip_if_incompatible_kernel_backend(KernelBackend.triton)
    torch.manual_seed(_SEED)

    state_head_dim = problem_shape[0]
    num_heads = max(problem_shape[1:])

    kernel, torch_ref = _get_op_level_tensors(device, dtype, problem_shape, batch_size=4, sequence_length=32)

    h0_kernel = h0_torch = None
    if has_input_state:
        h0_kernel, h0_torch = get_duplicated_tensors((4, num_heads, state_head_dim), device=device, dtype=dtype)

    gru_fn = torch.compile(gru, fullgraph=True) if is_compiling else gru

    y_kernel, _ = gru_fn(**kernel, input_state=h0_kernel, kernel_backend=KernelBackend.triton)
    y_torch, _ = gru(**torch_ref, input_state=h0_torch, kernel_backend=KernelBackend.torch)

    y_kernel.sum().backward()
    y_torch.sum().backward()

    assert_equal_tensors(y_kernel, y_torch, False)
    for name in kernel:
        assert_equal_tensors(kernel[name].grad, torch_ref[name].grad, False)

    if has_input_state:
        assert_equal_tensors(h0_kernel.grad, h0_torch.grad, False)


@pytest.mark.parametrize("is_compiling", [False, True])
@pytest.mark.parametrize("has_input_state", [False, True])
@pytest.mark.parametrize("problem_shape", _PROBLEM_SHAPES)
@pytest.mark.parametrize("dtype", _DTYPES)
@torch._dynamo.config.patch(recompile_limit=1024)
def test_op_varlen_kernel_vs_torch(
    dtype: torch.dtype,
    problem_shape: tuple[int, int, int, int, int, int, int],
    has_input_state: bool,
    is_compiling: bool,
) -> None:
    device = skip_if_incompatible_kernel_backend(KernelBackend.triton)
    torch.manual_seed(_SEED)

    (
        state_head_dim,
        num_input_heads,
        num_forget_input_heads,
        num_reset_input_heads,
        num_weight_heads,
        num_forget_weight_heads,
        num_reset_weight_heads,
    ) = problem_shape
    num_heads = max(problem_shape[1:])

    cu_seqlens = torch.tensor([0, 7, 19, 27, 93], device=device)
    max_seqlen = (cu_seqlens[1:] - cu_seqlens[:-1]).max().item()
    B = cu_seqlens.size(0) - 1
    total_tokens = int(cu_seqlens[-1])

    x_kernel, x_torch = get_duplicated_tensors(
        (total_tokens, num_input_heads, state_head_dim), device=device, dtype=dtype
    )
    xf_kernel, xf_torch = get_duplicated_tensors(
        (total_tokens, num_forget_input_heads, state_head_dim), device=device, dtype=dtype
    )
    xr_kernel, xr_torch = get_duplicated_tensors(
        (total_tokens, num_reset_input_heads, state_head_dim), device=device, dtype=dtype
    )
    W_kernel, W_torch = get_duplicated_tensors(
        (num_weight_heads, state_head_dim, state_head_dim), device=device, dtype=dtype
    )
    Wf_kernel, Wf_torch = get_duplicated_tensors(
        (num_forget_weight_heads, state_head_dim, state_head_dim), device=device, dtype=dtype
    )
    Wr_kernel, Wr_torch = get_duplicated_tensors(
        (num_reset_weight_heads, state_head_dim, state_head_dim), device=device, dtype=dtype
    )

    h0_kernel = h0_torch = None
    if has_input_state:
        h0_kernel, h0_torch = get_duplicated_tensors((B, num_heads, state_head_dim), device=device, dtype=dtype)

    gru_fn = torch.compile(gru, fullgraph=True) if is_compiling else gru

    y_kernel, h_kernel = gru_fn(
        input=x_kernel,
        weight=W_kernel,
        forget_input=xf_kernel,
        forget_weight=Wf_kernel,
        reset_input=xr_kernel,
        reset_weight=Wr_kernel,
        input_state=h0_kernel,
        cu_seqlens=cu_seqlens,
        max_seqlen=max_seqlen,
        kernel_backend=KernelBackend.triton,
    )

    y_torch = []
    h_torch = []
    for i in range(B):
        y, h = gru(
            input=x_torch[cu_seqlens[i] : cu_seqlens[i + 1]].unsqueeze(0),
            weight=W_torch,
            forget_input=xf_torch[cu_seqlens[i] : cu_seqlens[i + 1]].unsqueeze(0),
            forget_weight=Wf_torch,
            reset_input=xr_torch[cu_seqlens[i] : cu_seqlens[i + 1]].unsqueeze(0),
            reset_weight=Wr_torch,
            input_state=h0_torch[i].unsqueeze(0) if has_input_state else None,
            kernel_backend=KernelBackend.torch,
        )
        y_torch.append(y.squeeze(0))
        h_torch.append(h)

    y_torch = torch.cat(y_torch)
    h_torch = torch.cat(h_torch)

    assert_equal_tensors(y_kernel, y_torch, False)
    assert_equal_tensors(h_kernel, h_torch, False)
