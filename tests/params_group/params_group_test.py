...
from lm_engine.training.enums import ParamsGroupMethod
...
from lm_engine.training.parameter import get_parameter_marker_maps, set_parameter_marker_maps
...
@pytest.mark.parametrize("use_torch_compile", [False, True])
@pytest.mark.parametrize("efficient_initialization", [False, True])
def test_hyperball_params_group(
    use_fsdp: bool,
    use_torch_compile: bool,
    efficient_initialization: bool,
) -> None:
    """mirrors test_params_group, but for ParamsGroupMethod.hyperball - covers the
    conv_hyperball / attention_hyperball / hyperball routing in get_hyperball_group_with_names,
    including DeltaMLP's kv_conv1d.weight (mark_parameter_as_conv_hyperball) and softmax
    attention's c_attn.weight (is_attention_parameter)."""

    args = load_training_args_for_unit_tests("params_group/hyperball_training_config.yml")
    args.distributed_args.torch_compile = use_torch_compile
    args.model_args.efficient_initialization = efficient_initialization
    ...
    params_groups = get_param_groups_list(
        model_container, args.optimizer_args.class_args, ParamsGroupMethod.hyperball
    )[0]

    expected_group = json.load(open(os.path.join(os.path.dirname(__file__), "groups", "hyperball.json"), "r"))
