# **************************************************
# Copyright (c) 2026, Mayank Mishra
# **************************************************

import pytest
import torch

from lm_engine.training.modeling_utils import get_activation_function
from lm_engine.training.modeling_utils.activations import get_base_activation
from lm_engine.training.modeling_utils.activations.glu import GLUActivation

from .utils import assert_equal_tensors, skip_test_if_device_unavailable


@pytest.mark.parametrize("device", [torch.device("cpu"), torch.device("cuda")])
def test_sigmoid_glu(device: torch.device) -> None:
    skip_test_if_device_unavailable(device)

    base_sigmoid = get_base_activation("sigmoid")
    sigmoid_glu = GLUActivation(base_sigmoid)

    pytorch_glu = get_activation_function("glu")

    x = torch.randn(10, 10, device=device)
    sigmoid_glu_output = sigmoid_glu(x)
    pytorch_glu_output = pytorch_glu(x)

    assert_equal_tensors(sigmoid_glu_output, pytorch_glu_output, True)
